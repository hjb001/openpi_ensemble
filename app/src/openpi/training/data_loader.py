...
class DataLoaderImpl(DataLoader):
    ...
    def __iter__(self):
        for batch in self._data_loader:
            # REMOVE_ME: debug print normalized state (search REMOVE_ME_norm_state_debug in repo)
            st = np.asarray(batch["state"])
            s0 = st[0].reshape(-1) if st.ndim > 1 else st.reshape(-1)
            print("train norm_state[:21]", s0[:21])
            # end REMOVE_ME_norm_state_debug
